...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var samplesSubject = samples.filter(sampleObj => sampleObj.id == sample);
    //  5. Create a variable that holds the first sample in the array.
    var result = samplesSubject[0];
    ...
    var otuLabelsTopTen = result.otu_labels.slice(0, 10).reverse();;
    var sampleValuesTopTen = result.sample_values.slice(0,10).reverse();
 ...
 // 1. Create a variable that filters the metadata array for the object with the desired sample number.
  ...
  var metadataArray = metadata.filter(metaObj => metaObj.id == sample);
    ...
    var gaugeData = [{
      ...
      type: "indicator",
      mode: "gauge+number",
      title: {text: "<b>Belly Button <br />Washing Frequency</b> <br></br> Scrubs Per Week"},
      gauge: {
